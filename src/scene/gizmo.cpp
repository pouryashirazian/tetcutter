...
//////////////////////////////////////////////////////////
int GizmoInterface::setAxis(const ps::base::Ray &r) {
    //1 = x, 2 = y, 3 = z
...
int GizmoTranslate::intersect(const Ray &r) {
    if(m_x.intersect(r))
        return 1;
...
int GizmoScale::intersect(const ps::base::Ray &r) {
    if(m_x.intersect(r))
        return 1;
    else if(m_y.intersect(r))
        return 2;
...
int GizmoRotate::intersect(const Ray &r) {
    if(m_x.intersect(r))
        return 1;
    else if(m_y.intersect(r))
        return 2;
...
int GizmoRotate::setAxis(const Ray &r) {
    //1 = x, 2 = y, 3 = z
    int res = this->intersect(r);
    if(res > 0)
...
GizmoManager::GizmoManager():SGNode() {
    ...
    TheEngine::Instance().headers()->addHeaderLine("gizmo", "");
...
void GizmoManager::mousePress(MouseButton button, MouseButtonState state, int x, int y) {
    ...
    else if(button == MouseButton::mbLeft) {
        if(state == mbsPressed) {
            for(U32 i=0; i<m_clients.size(); i++)
                m_clients[i]->onStart();
        }
        else if(state == mbsReleased) {
            for(U32 i=0; i<m_clients.size(); i++)
                m_clients[i]->onStop();
        }
    }
...
void GizmoManager::mouseMove(int x, int y) {
    ...
    if(m_button != ps::mbLeft)
        return;
    if(m_buttonState != ps::mbsPressed)
        return;
    ...
    Ray r1 = TheEngine::Instance().screenToWorldRay(m_pressedPos.x, m_pressedPos.y);
    Ray r2 = TheEngine::Instance().screenToWorldRay(x, y);
...
void GizmoManager::mouseWheel(MouseWheelDir dir) {
    //for(U32 i=0; i<m_clients.size(); i++)
        //m_clients[i]->mouseWheel(dir);
}
...
void GizmoManager::cmdTranslate(const vec3f& increment) {
    ...
    TheEngine::Instance().headers()->updateHeaderLine("gizmo", buffer);
}
