...
void SGBox::setup(const vec3f& lo, const vec3f& hi) {
	...
	*/
	...
	//Setup Mesh
    m_meshbuffer.setupVertexAttribs(arrVertices, 3, gbtPosition);
    m_meshbuffer.setupVertexAttribs(arrNormals, 3, gbtNormal);
    m_meshbuffer.setupVertexAttribs(arrColors, 4, gbtColor);
    m_meshbuffer.setupFaceIndices(m_meshbuffer.countVertices(), ftQuads);
}
