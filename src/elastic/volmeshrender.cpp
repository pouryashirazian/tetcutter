...
#include "elastic/volmeshrender.h"
...
bool VolMeshRender::sync(const VolMesh* pmesh) {
	...
	//get camera position
    vec3f cp = TheEngine::Instance().camera().pos();
	...
	//setup surface mesh
    m_meshbuffer.setupVertexAttribsT<double>(GL_DOUBLE, vFlatNodes, 3, gbtPosition);
    m_meshbuffer.setupVertexAttribsT<double>(GL_DOUBLE, vFlatNodeNormals, 3, gbtNormal);
    m_meshbuffer.setupPerVertexColorT<float>(GL_FLOAT, pmesh->getColor(), pmesh->countNodes(), 3);
    m_meshbuffer.setupFaceIndexBufferT<U32>(GL_UNSIGNED_INT, vIndices, ftTriangles);

	//setup wireframe
    m_sgWireFrame.glmesh().clearAllBuffers();
    m_sgWireFrame.glmesh().setupVertexAttribsT<double>(GL_DOUBLE, vFlatNodes, 3, gbtPosition);
    m_sgWireFrame.glmesh().setupPerVertexColorT<float>(GL_FLOAT, Color(0, 0, 0, 100), pmesh->countNodes(), 4);
    m_sgWireFrame.glmesh().setupFaceIndexBufferT<U32>(GL_UNSIGNED_INT, vIndices, ftTriangles);
    m_sgWireFrame.glmesh().setWireFrameMode(true);

	//setup nodes
    m_sgVertices.glmesh().clearAllBuffers();
    m_sgVertices.glmesh().setupVertexAttribsT<double>(GL_DOUBLE, vFlatNodes, 3, gbtPosition);
    m_sgVertices.glmesh().setupPerVertexColorT<float>(GL_FLOAT, Color::red(), pmesh->countNodes(), 3);
    m_sgVertices.glmesh().setFaceMode(GLFaceType::ftPoints);
...
void VolMeshRender::draw() {
    ...
    if(peff) {
    	peff->bind();
        peff->setCamPos(TheEngine::Instance().camera().pos());
    }
