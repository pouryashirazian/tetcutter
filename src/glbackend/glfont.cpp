...
bool GLFont::setup(const string &font_name) {
    ...
    if (FT_Init_FreeType(&ft)) {
        vlogerror("FREETYPE: Could not init FreeType Library");
        return false;
    }
    ...
    for (GLubyte c = 0; c < 128; c++)
    {
        ...
        {
            vlogerror("FREETYTPE: Failed to load Glyph");
            continue;
        }
        // Generate texture
        GLuint texture;
        ...
        m_alphabet.insert(std::pair<GLchar, Character>(c, character));
    }
